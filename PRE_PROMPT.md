...
- `nginx`:
  ...
  - expone puertos `8080:80` y `8443:443` en este servidor; el puerto exacto puede cambiar en otras copias del proyecto según el host de despliegue
  - depende de `backend`
...
- `backend`:
  ...
  - escucha internamente en `3000`, pero el host lo expone como `3001:3000` en este servidor
...
- `mariadb`:
  ...
  - expone `3307:3306` en este servidor y puede variar en otros hosts

> Aclaración de despliegue: este repositorio se sincroniza como copia de seguridad y puede levantarse en más de un equipo o servidor. Por eso los puertos no son un valor único universal: en este servidor de producción/local se usa `http://192.168.1.21:8080/` para el frontend, mientras que en otra máquina del equipo puede ser `http://localhost:8080/` si mantiene el mismo mapeo. La regla general es: el servicio interno sigue siendo `80` para nginx y `3000` para Node, pero el puerto del host depende del equipo donde se despliega.
