...
const App = {
    ...
    construirUI: function (fechaExcepcion = null) {
        console.log("Construyendo UI. Fecha de excepción para el calendario:", fechaExcepcion);
        ...
        // Pasamos la fecha de excepción a la inicialización del calendario
    ...
    decidirEstadoInicial: function () {
        ...
        if (idFromUrl) {
            ...
            // --- ¡CAMBIO CLAVE! ---
            // Usamos una función flecha '() => {}' para que 'this' dentro de ella
            // siga siendo el objeto App.
            fetch(`/api/solicitudes/${this.solicitudId}`)
                .then(res => res.json())
                .then(solicitudData => { // <-- Función Flecha
                    if (solicitudData) {
        ...
        if (tipoId && cantidad && duracion && fechaSeleccionada) {
            ...
            if (reglasAplicables.length > 0) {
                ...
                if (depositoGarantia > 0) {
                    this.elements.presupuestoTotalDiv.innerHTML += `<div style="font-size: 0.6em; color: #0056b3; margin-top: 5px;"> Depósito: $${this.convertirNumero(depositoGarantia)}</div>`;
                }
