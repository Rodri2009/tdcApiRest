(function () {
    ...
    async function authenticateAndGetToken() {
        try {
            const storedToken = localStorage.getItem('mpAuthToken');
            if (storedToken) {
                authToken = storedToken;
                console.log('[Auth] ✅ Reutilizando token MP guardado');
                return authToken;
            }
    ...
    async function fetchWithAuth(url, options = {}, retryOn401 = true) {
        const token = authToken || localStorage.getItem('mpAuthToken') || await authenticateAndGetToken();
        if (!token) {
