...
BEGIN
    ...
    WHERE TABLE_SCHEMA = 'tdc_db' AND TABLE_NAME = 'solicitudes_bandas_backup';
    
    IF v_table_exists = 0 THEN
        -- Paso 1: Crear tabla temporal con estructura correcta
        CREATE TABLE solicitudes_bandas_backup (
        ...
        -- Paso 2: Copiar datos (evitando AUTO_INCREMENT)
        INSERT INTO solicitudes_bandas_backup 
        SELECT * FROM solicitudes_bandas;
        
        SELECT 'Tabla backup creada con datos copiados' AS info;
    ELSE
        SELECT 'Tabla backup ya existe, usando existente' AS info;
    END IF;
END$$
