...
if grep -R "fechas_bandas_confirmadas" --exclude-dir=database/migrations --exclude=REFACTORIZACION_SOLICITUDES.md --exclude=README.md --exclude=database/01_schema.sql --exclude=database/03_test_data.sql -n . | wc -l | grep -q "0"; then
  echo "OK"
else
  echo "FOUND"; grep -R "fechas_bandas_confirmadas" --exclude-dir=database/migrations --exclude=REFACTORIZACION_SOLICITUDES.md --exclude=README.md --exclude=database/01_schema.sql --exclude=database/03_test_data.sql -n .
  errors=$((errors+1))
fi
