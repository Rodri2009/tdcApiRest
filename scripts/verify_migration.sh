...
if grep -R "fechas_bandas_confirmadas" --exclude-dir=database/migrations --exclude-dir=migrations --exclude-dir=.git --exclude=REFACTORIZACION_SOLICITUDES.md --exclude=README.md --exclude=01_schema.sql --exclude=03_test_data.sql --exclude=nginx.conf -n . | wc -l | grep -q "0"; then
  echo "OK"
else
  echo "FOUND"; grep -R "fechas_bandas_confirmadas" --exclude-dir=database/migrations --exclude-dir=migrations --exclude-dir=.git --exclude=REFACTORIZACION_SOLICITUDES.md --exclude=README.md --exclude=01_schema.sql --exclude=03_test_data.sql --exclude=nginx.conf -n .
  errors=$((errors+1))
fi
